<aside id="sidebar" class="sidebar">

    <ul class="sidebar-nav" id="sidebar-nav">

      <li class="nav-item">
        <a class="nav-link" href="/dashboard">
          <i class="bi bi-grid"></i>
          <span>Dashboard</span>
        </a>
      </li><!-- End Dashboard Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/dashboard/info-lowongan">
          <i class="bi bi-briefcase-fill"></i>
          <span>Info Lowongan</span>
        </a>
      </li><!-- End Info Lowongan Page Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/dashboard/rekrutmen">
          <i class="bi bi-person-badge"></i>
          <span>Rekrutmen</span>
        </a>
      </li><!-- End Rekrutmen Page Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/dashboard/agenda">
          <i class="bi bi-calendar-check"></i>
          <span>Agenda</span>
        </a>
      </li><!-- End Agenda Page Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/dashboard/perusahaan">
          <i class="bi bi-building"></i>
          <span>Perusahaan</span>
        </a>
      </li><!-- End Perusahaan Page Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/dashboard/user">
          <i class="bi bi-person-fill"></i>
          <span>User</span>
        </a>
      </li>
      <!-- End User Page Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/dashboard/other">
          <i class="bi bi-three-dots-vertical"></i>
          <span>Other</span>
        </a>
      </li>
      <!-- End Other Page Nav -->

    </ul>

  </aside><!-- End Sidebar-->